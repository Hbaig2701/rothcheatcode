...
const PRODUCT_CONFIG_VERSION = 52; // v52: Carrier penalty-free cap retargeted to tax-from-IRA only (Joshua W., ticket 2b5ff7a4). The cap previously restricted the entire conversion + tax-from-IRA combination, which was overly aggressive — at most carriers a Roth conversion is an intra-carrier Trad → Roth transfer that doesn't count against the penalty-free withdrawal allowance, only the tax dollars actually distributed out of the contract do. The cap now applies to taxesPaidFromIRA only (when respect_penalty_free_limit + tax_payment_source = from_ira + still in surrender period); any conversion tax above the cap is modeled as paid from external funds via the new YearlyResult.taxesPaidExternally field. Conversions are no longer shrunk by the cap, so affected clients will see LARGER per-year conversions and proportionally LARGER lifetime tax savings vs the v51 model. Story-mode card + dashboard tooltip surface the overflow when it fires.
