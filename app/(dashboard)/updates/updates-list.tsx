...
import { Star, Video, Sun, PenTool, FileText, Users, Copy, DollarSign, FileCheck, Printer, Shield, Table2, ArrowUpDown, MoveHorizontal, UserCog, Columns, Tags, HandCoins, Heart, Sparkles, GitCompare, SlidersHorizontal, MessageCircle, GraduationCap, Scale, BookOpen, Calculator, Wrench, TrendingUp, PiggyBank, Split, CalendarClock, LineChart, Building2, ShieldCheck } from 'lucide-react'
...
const updates = [
  {
    ...
    description: "You can now add your broker-dealer's or RIA's required disclosure language once and have it appear on every client-facing report you produce.\n\nGo to Settings → Business and fill in \"Compliance Disclosure\". Whatever you type there prints on the reports you hand to clients, so you don't have to add it by hand or explain its absence to your compliance team. Leave it blank and nothing prints.",
  ...
  },
  {
    id: '46',
    date: 'September 24, 2026',
    title: 'Grow Recurring Income Year Over Year',
    description: "The Recurring button in the Non-SSI Income section now takes an annual increase. Set a start age, an end age, an amount, and a percentage, and the software fills every year with the amount growing by that percentage.\n\nBefore, Recurring could only repeat one flat figure, so anything that rises over time — a pension with a COLA, rental income, dividends — had to be typed in year by year. Now it's one entry.",
  ...
  },
  {
    ...
    title: 'Five More Carrier Products',
    description: "New products in the Community library, ready to pick on any client with the real bonus, surrender schedule, free-withdrawal rules and state availability already filled in:\n\n* Knighthead Chartline Bonus 10\n* North American Secure Horizon Accelerator (with and without the strategy charge)\n* Delaware Life Momentum Growth\n* American Equity IncomeShield 10\n\nEach one is checked against the carrier's own illustration before we publish it. Find them under Settings → My Products → browse the Community library. If a product you write isn't there, send us the brochure and rate sheet and we'll build it.",
    category: 'New Feature',
    icon: Building2,
  },
  {
    ...
    title: 'Clearer Results Summary on the PDF',
    description: "The Results box on page 2 of the report now explains itself. Under \"Lifetime Wealth Before\" it reads \"Without a Roth conversion, after all taxes\", and under \"Lifetime Wealth After\" it reads \"With the Roth conversion, after all taxes\".\n\nAdvisors told us clients were looking at those two figures without knowing what was being compared. Now the page answers it before they ask.",
    category: 'Enhancement',
    icon: FileText,
  },
  {
    ...
    title: 'QLAC Modelling',
    description: "You can now show what a Qualified Longevity Annuity Contract does to a client's plan. Open Section 4 (Tax Data) and enter a QLAC premium, the age income starts, and the death benefit option.\n\nThat premium comes out of the IRA balance for RMD purposes, so the forced distributions in the client's seventies and eighties drop, and the income switches on later at the age you choose. The projection carries it through the year-by-year tables, the charts and the report, so you can put a QLAC and a Roth conversion side by side and show which does more.",
    category: 'New Feature',
    icon: CalendarClock,
  },
  {
    ...
    title: 'Tax on Conversion Column',
    description: "A new year-by-year column showing the federal and state tax attributable to that year's conversion in a single figure, on screen and on the PDF's Strategy Account Overview.\n\nIt sits alongside \"Tax from IRA\", which counts only the dollars actually pulled out of the annuity. When a client pays the conversion tax from a brokerage account, \"Tax from IRA\" reads zero — so this column makes sure the cost of the conversion is visible either way. Add it from Adjust Columns.",
    category: 'New Feature',
    icon: Table2,
  },
  {
    id: '41',
    date: 'September 10, 2026',
    title: 'Account Summary States the Age',
    description: "The Account Summary now says which age its \"Final\" figures are as of — \"Final Roth Value (as of age 95)\", and the same on Final Traditional IRA and Total Portfolio.\n\nIt follows whatever end age you've set on the client, so when you're walking someone through the numbers there's no ambiguity about the time frame you're illustrating.",
    category: 'Enhancement',
    icon: CalendarClock,
  },
  {
    id: '40',
    date: 'August 7, 2026',
    title: 'Laddered Products Follow Their Own Crediting Schedule',
    description: "Some annuities don't credit a level rate — they build slowly early on and compound harder in the last few years of the term. Those products now project on their real year-by-year schedule instead of one flat rate.\n\nBoth sides of the comparison ride the same schedule, so the do-nothing baseline and the conversion strategy stay apples-to-apples and the report isolates the tax decision rather than a return difference.",
    category: 'Enhancement',
    icon: LineChart,
  },
  {
    id: '39',
    date: 'July 24, 2026',
    title: 'No Annuity Mode',
    description: "A product preset called \"No Annuity (Roth Conversion Only)\" for clients where there's no annuity in the picture and you just want to show the conversion itself.\n\nPick it in Section 3 and the money simply compounds at the rate you set. Every annuity term disappears from the form, the report, Story Mode and the PDF — no bonus, no surrender schedule, no carrier language — so the client sees a clean Roth conversion illustration and nothing they'd have to ask about.",
    category: 'New Feature',
    icon: Scale,
  },
  {
    ...
    date: 'July 14, 2026',
    title: 'Refer & Earn',
    description: "You can now enrol yourself in the referral programme from Settings → Refer & Earn. You get your own link, your referrals get 20% off, and you earn 25% commission — around $594 a year for every advisor who signs up through you.\n\nThe page shows who's signed up, who's started and not finished, and what you've earned, so you can follow up with the ones still sitting on the fence.",
    category: 'New Feature',
    icon: HandCoins,
  },
  {
    id: '37',
    date: 'July 6, 2026',
    title: 'Hold Back Part of the IRA',
    description: "For clients who are only moving part of their IRA into the annuity, you can now model the rest properly instead of pretending it isn't there.\n\nEnter the amount going into the annuity as the Qualified Account Value, then in Section 4 (Tax Data) tick \"RMDs Handled Externally\" and enter the balance being left behind along with its own growth rate. The software works out the RMDs on that held-back money each year and stacks them into the client's income, so the conversion gets taxed in the right brackets.\n\nThere's also a new \"RMD (External)\" column so you can see those forced distributions year by year.",
    category: 'New Feature',
    icon: Split,
  },
  {
    id: '36',
    date: 'July 5, 2026',
    title: 'More Training Lessons',
    description: "Two new lessons in the Training Centre: \"Understanding Your Client's Results\", which walks through reading a finished report, and \"How to Submit a Support Ticket\", which shows how to get us what we need to answer you quickly.\n\nThe lesson list is now expandable, and videos only load when you open one, so the page comes up straight away.",
    category: 'Enhancement',
    icon: GraduationCap,
  },
  {
    id: '35',
    date: 'July 2, 2026',
    title: 'Net (After-Tax) Withdrawals',
    description: "Withdrawals in Section 8 can now be entered as the amount the client actually wants in their pocket, rather than the gross amount leaving the account.\n\nSet a row's Amount type to \"Net (after-tax)\" and the do-nothing side grosses up its IRA withdrawal to cover the tax that pull triggers — so if the client needs $50,000 to live on, the baseline pulls what it really takes to net that, and drains faster accordingly. The Roth side stays at $50,000 because that money is already after-tax.\n\nIt makes the two sides a fair comparison when you're modelling real spending.",
    category: 'New Feature',
    icon: PiggyBank,
  },
  {
