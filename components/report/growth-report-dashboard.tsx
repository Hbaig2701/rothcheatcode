...
export function GrowthReportDashboard({ client, projection }: GrowthReportDashboardProps) {
  ...
  // Held-back Traditional IRA RMDs (the external RMD column) — fold into the
  // RMD totals so the tooltip/Distributions narrative reflects ALL the client's
  // RMDs (converting slice + held-back IRA), not just the modeled slice. 0 when
  // there's no held-back balance, so non-held-back clients are unchanged.
