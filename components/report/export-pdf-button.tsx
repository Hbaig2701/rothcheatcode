...
/**
 * Export PDF Button for the 13-page RothBlueprint Report
 *
...
export function ExportPdfButton({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.download = `RothBlueprint_${sanitizedName}_${timestamp}.pdf`;
...
export function ExportPdfButtonCompact({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `RothBlueprint_${client.name.replace(/\s+/g, '_')}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
