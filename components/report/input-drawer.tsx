...
export function InputDrawer({ client, onClose }: InputDrawerProps) {
  ...
  useEffect(() => {
    ...
    if (matchesOtherLockedProduct) {
      // Only correct the LOCKED identity fields (carrier/product name). Do NOT
      // re-stamp the advisor-editable value fields (bonus / surrender /
      // penalty-free) — the preset locks penalty-free at its default, but the
      // advisor can override it (e.g. an Athene base at 5% vs the preset's 10%),
      // and re-applying the default here on mount clobbered that override back
      // to the preset value (Jorge Tola: penalty-free reverts 5% → 10%).
