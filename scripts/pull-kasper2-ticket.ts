...
(async () => {
  ...
  mkdirSync('/tmp/kasper2-ticket', { recursive: true });
  for (const a of atts ?? []) {
    ...
    writeFileSync(`/tmp/kasper2-ticket/${safe}`, Buffer.from(arr));
    console.log(`saved /tmp/kasper2-ticket/${safe} (${a.mime_type}, ${arr.byteLength} bytes)`);
  }
