...
export const COLUMN_DEFINITIONS: ColumnDefinition[] = [
  ...
  },
  {
    ...
    category: 'balances',
    description: "Taxable account balance. In the baseline, this accumulates after-tax RMD proceeds. In the strategy, it also carries the cumulative \"Held-Back RMD Advantage\" (see that column) when a held-back IRA is set — the extra after-tax money the strategy keeps because the held-back RMDs are taxed at a lower marginal rate.",
    formatter: formatCurrency,
    defaultVisible: false,
    visibleForProducts: ['all'],
    defaultWidth: 150,
  ...
  {
    id: 'heldBackResidual',
    label: 'Held-Back RMD Advantage',
    category: 'balances',
    description: "Cumulative after-tax advantage from the client's held-back Traditional IRA (strategy only). Its RMDs are taxed at a LOWER marginal rate under the strategy than under do-nothing (once the converted slice is Roth, there are no forced RMDs stacking underneath), so the client keeps more of each RMD after tax. This running total — reinvested and grown — is already included in the strategy's Taxable Account and Net Worth above; this column breaks it out so you can see it as its own line. The held-back IRA's balance itself is a wash (identical both scenarios) and is not in these figures. $0 on the baseline and whenever there's no held-back IRA.",
    formatter: formatCurrency,
    defaultVisible: false,
    visibleForProducts: ['all'],
    defaultWidth: 170,
  ...
  },
  {
    ...
    category: 'distributions',
    description: 'Total dollars that physically leave the Traditional IRA this year: the Roth conversion + RMDs/voluntary withdrawals + any conversion tax pulled BEYOND the RMD. When the carrier funds the conversion tax in an RMD year, that tax is withheld from the RMD (already included here), so it is not added on top — this number can be smaller than conversion + RMD + tax. This is the number to give the annuity carrier.',
    formatter: formatCurrency,
    defaultVisible: true,
    visibleForProducts: ['growth'],
    defaultWidth: 160,
  ...
  },
  {
    ...
    label: 'Std Deduction',
    category: 'taxes',
    description: 'Standard deduction applied to gross income. Increases at age 65+ for each spouse. Filing status (single vs MFJ) also affects the amount. This reduces taxable income before tax brackets apply. Note: clients 65+ may also get the Senior Deduction (see next column) — AGI minus BOTH deductions gives Taxable Income.',
    formatter: formatCurrency,
    defaultVisible: false,
    visibleForProducts: ['all'],
    defaultWidth: 140,
  ...
  },
  {
    ...
    category: 'taxes',
    description: 'Federal tax attributable to Roth conversions only. Calculated at the marginal rate — this is the "cost" of converting, which is recovered over time through tax-free Roth growth. Note: this excludes the tax owed on the gross-up dollars when tax_payment_source = "From IRA" — those land in "Fed Tax (Ordinary)" since the gross-up is itself a taxable IRA distribution. Use "Total Fed Tax on IRA Withdrawal" for the combined number.',
    formatter: formatCurrency,
    defaultVisible: false,
    visibleForProducts: ['all'],
    defaultWidth: 170,
  ...
  {
    id: 'federalTaxOnIRAWithdrawal',
    label: 'Total Fed Tax on IRA W/D',
    category: 'taxes',
    description: 'Marginal federal tax owed on the FULL year\'s IRA distribution: conversions + tax-from-IRA gross-up + RMDs + voluntary IRA withdrawals. Computed as the difference between the year\'s federal tax with vs without the IRA distribution. This is the single number an advisor wants when a client asks "what does it cost me in federal tax to pull X out of the IRA this year?" — the existing "Fed Tax (Conversions)" column splits that across two buckets, which is technically correct but confusing.',
    formatter: formatCurrency,
    defaultVisible: true,
    visibleForProducts: ['all'],
    defaultWidth: 200,
